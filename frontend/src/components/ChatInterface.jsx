...
import { getFriendFromChat } from "../utils/chatUtils";
...
const ChatInterface = ({ selectedChat }) => {
  ...
  const { isLoading, friendRequests, error } = useSelector(
    (state) => state.friends,
  );
  ...
  const selectedFriend =
    selectedChat && currentUserId != null
      ? getFriendFromChat(selectedChat, currentUserId)
      : null;
  const { socket, connectionStatus, STATUS } = useSocket();
  ...
  useEffect(() => {
    const handleEscape = (e) => {
      if (e.key === "Escape") setShowUpdateProfile(false);
    };
        ...
        <div className="flex items-center gap-3 min-w-[180px]">
          {activeTab === "chat" && selectedFriend && (
            <>
              <img
                src={
                  selectedFriend?.avatar?.secure_url || DEFAULT_AVATAR
                }
